...
export default function SignupPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            await signUp(email, password, displayName, weightNum);
            toast.success('Account created! Welcome to Hydration Tracker 💧');
    ...
    const handleGoogleSignup = async () => {
        ...
        try {
            ...
            toast.success('Welcome to Hydration Tracker! 💧');
            router.push('/dashboard');
        } catch (error: any) {
    ...
    const dailyGoal = parseFloat(weight) > 0 ? (parseFloat(weight) / 20).toFixed(1) : '0';
            ...
            <Card className="w-full max-w-md backdrop-blur-sm bg-white/90 dark:bg-slate-900/90 shadow-2xl border-0">
                <CardHeader className="space-y-4 text-center">
                    ...
                    <CardTitle className="text-3xl font-bold bg-gradient-to-r from-blue-600 to-cyan-600 bg-clip-text text-transparent">
                        Join Hydration Tracker
                    </CardTitle>
                    <CardDescription className="text-base">
                        Create your account and start tracking your hydration
                    </CardDescription>
                </CardHeader>
                        ...
                        <div className="space-y-2">
                            ...
                            {parseFloat(weight) > 0 && (
                                <p className="text-sm text-muted-foreground">
                                    Your daily goal: <span className="font-semibold text-blue-600">{dailyGoal}L</span> (1L per 20kg)
                                </p>
                            )}
                        </div>
